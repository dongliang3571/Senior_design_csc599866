...
// gray = .3R +.59G + .11B
void HistogramStretching::Stretching(ImagePtr I1, int max, int min, ImagePtr I2) {
    ...
    for (i = 0; i < MXGRAY; i++) {
        lut[i] = CLIP(MaxGray * (i - tempMin) / (tempMax - tempMin), 0, MaxGray);
    }
...
void HistogramStretching::settingSliderAndSpinBox(QSlider* slider, QSpinBox* spinbox, int value) {
